...
Prerequisites:
 - the `master` branch is green and it contains all the changes that should be 
 included in the next release.

Steps:
1. Create a new branch with the name `ATS-###_release_version` from the `master` branch.
...
been generated by steps (2) and (3), then an empty commit should be created - e.g.
     ```bash
     git commit --allow-empty -m "ATS-###: Release T-Model #.##.# [trigger release]"
     ```
...
`master` branch and wait for a green build; the **Release** stage on the PR build
